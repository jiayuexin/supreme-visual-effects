...
const neonStyle = computed(() => {
  const baseGlowIntensity = isHovered.value && props.hoverEffect 
    ? props.hoverGlowIntensity 
    : props.glowIntensity;
...
const generateTextShadow = (intensity: number) => {
  ...
  for (let i = 1; i <= steps; i++) {
    ...
    shadows.push(`0 0 ${blur}px ${props.glowColor}${Math.floor(opacity * 255).toString(16).padStart(2, '0')}`);
  }
...
watch(() => [props.animation, props.animationType, props.animationSpeed], () => {
  if (props.animation && props.animationType !== 'none') {
    startAnimation();
  } else {
    stopAnimation();
