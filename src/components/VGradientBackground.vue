...
const generateStaticGradient = () => {
  const gradients = props.layers.map((layer, index) => {
  ...
  const gradients = props.layers.map((layer, index) => {
    const stops = layer.stops.map(stop => {
      ...
      if (layer.animation) {
        ...
        const intensity = layer.animation.intensity || 0.3;
        ...
        switch (layer.animation.type) {
          ...
          case 'twinkle':
            // 闪烁效果：亮度变化
            const brightness = 0.8 + 0.4 * Math.sin(t * 3 + index);
    ...
    switch (layer.type) {
      case 'linear':
        const angle = layer.direction ? 
          parseFloat(layer.direction) + (time.value * 10) : 
          45 + (time.value * 10);
