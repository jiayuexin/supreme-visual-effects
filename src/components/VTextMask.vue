<template>
  <div class="text-mask-container" :style="containerStyle">
    <svg :width="svgWidth" :height="svgHeight" class="text-mask-svg" :style="svgStyle">
      <defs>
        <mask :id="maskId">
      ...
      <rect width="100%" height="100%" :fill="backgroundColor" :mask="`url(#${maskId})`" />
...
watch(
  () => [props.animation, props.duration, props.delay],
  () => {
    if (props.animation) {
      startAnimation()
    } else {
