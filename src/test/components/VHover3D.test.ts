...
describe('VHover3D', () => {
  it('should render correctly with default props', () => {
    const wrapper = mount(VHover3D, {
      slots: { default: '<div>Hover Content</div>' },
    })
    expect(wrapper.find('.hover-3d-container').exists()).toBe(true)
  ...
  it('should set correct scale', () => {
    const wrapper = mount(VHover3D, {
      props: { scale: 1.2 },
      slots: { default: 'Scaled Content' },
    })
    expect(wrapper.vm.scale).toBe(1.2)
  })

  it('should set correct perspective value', () => {
    const wrapper = mount(VHover3D, {
      props: { perspective: 1500 },
      slots: { default: 'Perspective Content' },
    })
    expect(wrapper.vm.perspective).toBe(1500)
  })
})
